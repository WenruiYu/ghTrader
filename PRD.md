...
- `ghtrader db refresh-main-l5-daily-agg` - Rebuild `main_l5` daily aggregate table for common coverage aggregations
...
- Enforce trading lock discipline for strategy execution (`trade:strategy:account=<PROFILE>`) while keeping gateway account lock independent, to prevent duplicate StrategyRunner writers without blocking same-account gateway+strategy concurrency.
...
- Started and completed `P1-T1` reliability hardening: gateway command dedupe is now bounded by TTL + capacity (configurable), with regression tests for duplicate suppression, TTL expiry, and capacity eviction.
- Started and completed `P1-T2` reliability hardening: strategy hot-path now uses an explicit ZMQ connection-health FSM (suspect/degraded/healthy transitions with bounded fail/recover thresholds), and gateway publishes periodic hot-path heartbeat state to reduce timeout flapping under sparse market updates.
- Started and completed `P1-T3` reliability hardening: gateway state now carries monotonic `state_revision` across sink writes, and control-plane readers now resolve Redis/file state via revision-first freshness selection (fallback `updated_at`) to improve cross-sink observer consistency under partial failures.
- Started and completed `P1-C3` reliability hardening: WebSocket dashboard broadcast path now tracks send failures and prunes dead clients deterministically to prevent repeated broadcast exceptions under disconnect storms.
- Started and completed `P1-D1` reliability hardening: main_l5 ingest now uses a maintenance-incident circuit breaker and emits explicit per-day skip ledger records so long provider maintenance windows no longer fail the full segment build.
- Started and completed `P1-D2` reliability hardening: `main_l5` enforce-health build gate now integrates `main_l5_validation` and blocks dataset promotion on validation `overall_state=error` with actionable `reason_code/action_hint` diagnostics captured in update reports.
- Started `P1-C1` maintainability refactor (phase 1): extracted websocket connection-manager + endpoint mounting from `control/app.py` into dedicated `control/websocket_manager.py` to reduce composition density and isolate realtime concerns under explicit regression coverage.
- Continued `P1-C1` maintainability refactor (phase 2): extracted data-quality route composition (`/api/data/quality/readiness|anomalies|profiles`) from `control/app.py` into dedicated `control/routes/quality.py` to further shrink app-level orchestration surface while preserving API contracts.
- Continued `P1-C1` maintainability refactor (phase 3): extracted heavier data quality query/read routes (`/api/data/quality-summary` and `/api/data/main-l5-validate*`) from `control/app.py` into dedicated `control/routes/quality_data.py`, keeping route contracts unchanged with module-boundary regression coverage.
- Continued `P1-C1` maintainability refactor (phase 4): extracted data report read routes (`/api/data/reports` and `/api/data/l5-start`) from `control/app.py` into dedicated `control/routes/data_reports.py`, preserving API contracts with dedicated module-boundary regression coverage.
- Continued `P1-C1` maintainability refactor (phase 5/6): extracted data enqueue write routes (`/api/data/enqueue-l5-start`, `/api/data/enqueue-main-l5-validate`) into `control/routes/data_jobs.py`, and moved read-only QuestDB SQL endpoint (`/api/questdb/query`) into `control/routes/questdb_query.py` with route-module regression coverage.
- Continued `P1-C1` maintainability refactor (phase 7/8): extracted dashboard status APIs (`/api/dashboard/summary`, `/api/ui/status`) into `control/routes/dashboard_status.py`, and extracted strategy/trading runtime APIs (`/api/strategy/*`, `/api/gateway/start|stop`, `/api/trading/console/status`) into `control/routes/trading_runtime.py` with module-boundary regression coverage.
- Continued `P1-C1` maintainability refactor (phase 9): extracted duplicated control-plane helper logic into shared `control/app_helpers.py` (variety normalization, job-variety filtering, model artifact scanning, dashboard guardrail state payload), and rewired `app.py`/`routes/models.py`/`routes/jobs.py`/`views.py` to consume the shared helpers for consistency and lower drift risk.
- Continued `P1-C1` maintainability refactor (phase 10): extracted control bootstrap responsibilities from `control/app.py` into dedicated `control/bootstrap.py` (TqSdk-heavy job classification, scheduler/supervisor enable/start/tick wiring, control runtime path helpers), rewired `create_app` to compose bootstrap helpers directly, and migrated supervisor tick regression tests to verify the new module boundary.
- Continued `P1-C1` maintainability refactor (phase 11): extracted full FastAPI assembly (`create_app`) from `control/app.py` into dedicated `control/app_factory.py`, reduced `control/app.py` to a compatibility shim (`create_app` + module-level `app` entrypoint + websocket compatibility symbols), and added factory-boundary regression coverage to prevent composition logic from drifting back into `app.py`.
- Continued `P1-C1` maintainability refactor (phase 12): extracted route-assembly composition from `control/app_factory.py` into dedicated `control/route_mounts.py`, reduced `create_app` to high-level lifecycle steps (runtime init, middleware wiring, background workers, static mount, route mount), and added dedicated app-factory/bootstrap boundary regression coverage.
- Completed `P1-C1` maintainability refactor (phase 13 closure): confirmed `control/app.py` remains a pure compatibility shim and added explicit architecture guard regression (`tests/test_control_app_factory.py::test_control_app_module_is_compatibility_shim`) to prevent composition logic from drifting back into the shim module.
- Implemented `P1-T3` follow-up consistency hardening: `GatewayWriter` now rehydrates persisted `state_revision` on restart and exports Redis state-write failure counters/streak telemetry; gateway loop maps sustained Redis state-write failures to explicit warm-path degraded reason (`redis_state_write_failed`) while preserving fail-open runtime behavior.
- Implemented `P1-C2` supervisor telemetry baseline: supervisor ticks now emit restart/stop reason attribution (`restart_reasons_total` / `stop_reasons_total`), attach restart reason metadata to supervisor-started jobs, expose supervisor telemetry via app state for API consumption, and surface this telemetry in SLO snapshots and Trading Console status payloads.
- Started `P2-A1` (`control/views.py` decomposition): extracted pipeline/calendar helper block (`_l5_calendar_context`, `_pipeline_guardrails_context`, `_validation_profile_suggestion`, `_pipeline_health_context`) into dedicated `control/views_pipeline_helpers.py`, rewired `views.py` to import shared helpers, and added module-boundary regression coverage.
- Continued `P2-A1` (`control/views.py` decomposition): extracted training-launch orchestration helpers (`_build_train_job_argv` core policy + active DDP-train detection) into dedicated `control/views_training_helpers.py`, kept `views.py` as thin wrappers around the helper module, and added regression coverage in `tests/test_control_views_training_helpers.py`.
- Continued `P2-A1` (`control/views.py` decomposition): extracted model/eval action route handlers (`/models/model/*` + `/models/eval/*`) into dedicated `control/views_model_actions.py`, rewired `views.py` to mount these routes via `register_model_action_routes`, and added route-module boundary regression coverage in `tests/test_control_views_model_actions.py`.
- Continued `P2-A1` (`control/views.py` decomposition): extracted jobs action handlers (`/jobs/start`, `/jobs/{job_id}/cancel`) into dedicated `control/views_job_actions.py`, rewired `views.py` to mount via `register_job_action_routes`, and simplified repeated model-action job-start/redirect flow in `views_model_actions.py` while preserving route behavior under regression coverage (`tests/test_control_views_job_actions.py`, `tests/test_control_main_build_dedupe.py`).
- Continued `P2-A1` (`control/views.py` decomposition): extracted Data Hub page routing/aggregation block (`/data`, `/v/{variety}/data`) and its cache/query orchestration into dedicated `control/views_data_page.py`, rewired `views.py` to mount via `register_data_page_routes`, and migrated data-page cache clear path to the new module boundary (including `route_mounts` wiring) with route-module regression coverage (`tests/test_control_views_data_page.py`).
- Continued `P2-A1` (`control/views.py` decomposition): extracted Data Hub loader internals (reports/QuestDB status/coverage/validation snapshot loaders) from `control/views_data_page.py` into dedicated `control/views_data_page_loaders.py`, rewired data-page orchestration to call shared loaders, and added loader-boundary regression coverage (`tests/test_control_views_data_page_loaders.py`).
- Continued `P2-A1` (`control/views.py` decomposition): extracted Data Hub orchestration internals (cache fan-out scheduling + async loader dispatch + snapshot merge/fallback) from `control/views_data_page.py` into dedicated `control/views_data_page_orchestrator.py`, kept route layer focused on request/render composition, and added orchestrator-boundary regression coverage (`tests/test_control_views_data_page_orchestrator.py`).
- Continued `P2-A1` follow-up hardening: normalized Data Hub orchestrator fallback payloads so loader exceptions are surfaced to UI-facing `coverage_error` / `validation_error` fields, corrected `cache_hits` timing metric semantics to count preload hits before async cache fill, extended orchestrator regression coverage for both behaviors, and cleaned residual unused imports in `control/views.py`.
- Continued `P2-A1` (`control/views.py` decomposition): extracted Data Hub action routes (`/data/integrity/report/*`, `/data/main-l5-validate/report/*`, `/data/settings/tqsdk_scheduler`, `/data/build/*`) into dedicated `control/views_data_actions.py`, rewired `views.py` to mount via `register_data_action_routes`, and added route-module boundary regression coverage (`tests/test_control_views_data_actions.py`) while preserving existing build dedupe/variety-context route behavior tests.
- Continued `P2-A1` (`control/views.py` decomposition): extracted workspace page routes (`/`, `/v/{variety}/dashboard`, `/jobs`, `/v/{variety}/jobs`, `/models`, `/v/{variety}/models`, `/trading`, `/v/{variety}/trading`) into dedicated `control/views_workspace_pages.py`, rewired `views.py` to mount via `register_workspace_page_routes`, and added route-module boundary regression coverage (`tests/test_control_views_workspace_pages.py`).
- Continued `P2-A1` (`control/views.py` decomposition): extracted job detail page route (`/jobs/{job_id}`) into dedicated `control/views_job_detail.py`, rewired `views.py` to mount via `register_job_detail_routes`, and added route-module boundary regression coverage (`tests/test_control_views_job_detail.py`).
- Completed `P2-A1` (`control/views.py` decomposition): `views.py` now serves as composition/registration layer only (no inline page/action route handlers), with module-boundary and route-behavior regressions passing for decomposed route families.
- Completed `P2-A2` (query budget + pagination standards across control APIs): introduced shared query-budget helpers in `control/routes/query_budget.py` and standardized limit/offset/max-bytes handling across `routes/jobs.py`, `core.py`, `questdb_query.py`, `data_reports.py`, `quality.py`, `quality_data.py`, `models.py`, `gateway.py`, and `trading_runtime.py`; also fixed `models inventory` `max_rows` enforcement and added dedicated regression coverage in `tests/test_control_query_budget_pagination.py`.
- Completed `P2-D1` (data contract checks in CI): added deterministic contract utility (`data/contract_checks.py`) for schema/null-rate/row-hash validation, wired CLI preflight command (`ghtrader data contract-check`), added regression tests (`tests/test_data_contract_checks.py`), and enforced preflight via dedicated CI `data-contract` job.
- Completed `P2-O2` (full end-to-end pipeline integration tests): added `tests/test_e2e_data_pipeline.py` with deterministic daily-pipeline orchestration coverage plus optional live provider smoke (guarded by explicit CI flag/credentials), and added dedicated CI `e2e-pipeline` job.
- Completed `P2-O1` (incident response runbook + SLO thresholds): expanded `reports/data_quality_operations_manual.md` with explicit SLO thresholds/drill checklist and added `reports/incident_response_runbook.md` covering QuestDB outage/data-contract/main_l5-validation/model-regression response playbooks.
- Completed `P2-O3` (coverage governance cleanup): added documented exclusion rationale matrix (`reports/coverage_governance_review.md`), added enforcement test (`tests/test_coverage_governance.py`), and linked `pyproject.toml` coverage exclusions to this governance baseline.
- P2 retrospective hardening (2026-02-24): consolidated 7 duplicated `_now_iso()` into shared `util/time.now_iso`; extracted shared `null_rate()` to `ticks_schema.py`; extracted duplicated progress helpers to `data/progress_helpers.py`; extracted `_variety_nav_ctx`/`_job_summary`/`_start_or_reuse_job_by_command` from `views.py` into `views_helpers.py` (views.py now 268 lines); added 4 missing contract-check test branches; fixed stale `test_data_page_cache.py` module reference; cleaned dead imports across 4 files. Full suite: 397 passed.
- Completed `P1-M2` (models training orchestration split): moved `train_model` implementation from `research/models.py` into dedicated `research/models_training.py`, kept `research/models.py` as backward-compatible facade (`create_model`/`load_model`/`train_model` API unchanged), and verified with targeted model/e2e regressions plus full non-integration suite.
- Completed `P1-C1` (app composition boundary finalization): extracted remaining app lifecycle responsibilities from `control/app_factory.py` into dedicated `control/app_lifecycle.py` (runtime components, scheduler settings load, HTTP observability middleware, runtime state attach, background worker startup), keeping `app_factory` as high-level composition only and extending app-factory boundary tests.
- Completed QuestDB query/perf hardening (2026-02-24): added configurable tick table partition strategy (`GHTRADER_QDB_TICKS_PARTITION_BY` supports `DAY`/`MONTH`/`YEAR`, default `DAY`) in `questdb/serving_db.py`; removed cast-based `trading_day` predicates in hot query paths (`fetch_ticks_for_day`, `list_trading_days_for_symbol`, `query_symbol_recent_last`) to improve predicate efficiency; added regression coverage in `tests/test_questdb_query_sql_filters.py` and extended `tests/test_questdb_sender_compat.py`; full non-integration suite remains green.
- Completed QuestDB `trading_day` predicate normalization follow-up (2026-02-24): removed remaining cast-based `trading_day` predicates/order clauses across `questdb/main_l5_validate.py`, `questdb/main_schedule.py`, `data/gap_detection.py`, and `questdb/queries.py` hot freshness paths (`query_symbol_latest`/`query_symbol_recent_last`), updated SQL Explorer sample query to direct `trading_day` aggregations, and added module-level SQL regression coverage in `tests/test_questdb_trading_day_predicates.py` plus updated cleanup strategy assertion (`tests/test_questdb_row_cleanup_strategy.py`); full non-integration suite remains green.
- Started QuestDB common-aggregation materialization (2026-02-23): added daily aggregate table support (`ghtrader_main_l5_daily_agg_v2`) in `questdb/main_l5_daily_agg.py` with idempotent DDL + rebuild workflow, wired dashboard symbol-count query to prefer aggregate-path with raw-table fallback (`control/routes/dashboard_status.py`), routed `query_symbol_day_bounds`/`query_symbol_latest`/`query_symbol_recent_last` to prefer daily-aggregate reads and only fallback to raw-table scans for missing symbols (`questdb/queries.py`), and integrated build-path auto-refresh in `data/main_l5.py` (best-effort, configurable via `GHTRADER_MAIN_L5_DAILY_AGG_AUTO_REFRESH`, with report telemetry and safe skip/error degradation); maintenance CLI command `ghtrader db refresh-main-l5-daily-agg` remains available for manual backfill. Regression coverage expanded in `tests/test_main_l5_daily_agg.py` + `tests/test_questdb_daily_agg_path.py` + `tests/test_main_l5_update.py`.

**Database and Performance**:
- Optimize QuestDB partitioning for typical query patterns
- Add materialized views for common aggregations (in progress: `main_l5` daily aggregate foundation landed)
