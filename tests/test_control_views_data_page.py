...
def test_views_register_data_page_routes_is_loaded_from_dedicated_module() -> None:
    views_mod = importlib.import_module("ghtrader.control.views")
    assert (
        getattr(getattr(views_mod, "register_data_page_routes"), "__module__", "")
        == "ghtrader.control.views_data_page"
    )


def test_views_data_page_routes_are_mounted_from_dedicated_module(
    ...
    assert all(mod == "ghtrader.control.views_data_page" for mod in route_modules.values())
