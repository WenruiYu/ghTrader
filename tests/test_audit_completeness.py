...
def test_audit_completeness_reports_missing_days(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from ghtrader.data.audit import audit_completeness
    ...
    monkeypatch.setattr(
        "ghtrader.data.trading_calendar.get_trading_days",
    ...
    monkeypatch.setattr("ghtrader.questdb.index.query_contract_coverage_from_index", fake_cov)
    monkeypatch.setattr("ghtrader.questdb.index.list_no_data_trading_days", lambda **_kwargs: [date(2020, 1, 8)])
