...
def test_views_register_job_action_routes_is_loaded_from_dedicated_module() -> None:
    views_mod = importlib.import_module("ghtrader.control.views")
    assert (
        getattr(getattr(views_mod, "register_job_action_routes"), "__module__", "")
        == "ghtrader.control.views_job_actions"
    )


def test_views_job_action_routes_are_mounted_from_dedicated_module(
    ...
    assert all(mod == "ghtrader.control.views_job_actions" for mod in route_modules.values())
