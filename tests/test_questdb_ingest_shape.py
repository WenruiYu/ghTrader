...
def test_questdb_df_for_ticks_shape_and_row_hash_changes(small_synthetic_tick_df: pd.DataFrame) -> None:
    from ghtrader.tq.ingest import _questdb_df_for_ticks

    df = small_synthetic_tick_df.head(5).copy()
    td = date(2025, 1, 1)

    out1 = _questdb_df_for_ticks(df=df, trading_day=td, ticks_kind="raw", dataset_version="v2")
    assert len(out1) == len(df)

    # Required columns for ILP ingestion
    for c in ["symbol", "datetime_ns", "ts", "trading_day", "row_hash", "dataset_version", "ticks_kind", "last_price"]:
    ...
    assert out1["ticks_kind"].nunique() == 1 and out1["ticks_kind"].iloc[0] == "raw"

    assert out1["datetime_ns"].astype("int64").tolist() == df["datetime"].astype("int64").tolist()

    # Deterministic hashing for stable inputs
    out2 = _questdb_df_for_ticks(df=df, trading_day=td, ticks_kind="raw", dataset_version="v2")
    ...
    out3 = _questdb_df_for_ticks(df=df2, trading_day=td, ticks_kind="raw", dataset_version="v2")
