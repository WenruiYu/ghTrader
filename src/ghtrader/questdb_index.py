...
from ghtrader.questdb_client import QuestDBQueryConfig, connect_pg as _connect
...
def ensure_index_tables(
    *,
    ...
    CREATE TABLE IF NOT EXISTS {idx} (
      ...
      DEDUP UPSERT KEYS(ts, symbol, trading_day, ticks_lake, lake_version)
    """
    ...
    try:
        with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
            ...
            with conn.cursor() as cur:
                ...
                try:
                    cur.execute(f"ALTER TABLE {idx} DEDUP ENABLE UPSERT KEYS(ts, symbol, trading_day, ticks_lake, lake_version)")
                except Exception:
                    pass
                try:
                    cur.execute(f"ALTER TABLE {nd} DEDUP ENABLE UPSERT KEYS(ts, symbol, trading_day, ticks_lake, lake_version)")
                except Exception:
                    pass
...
def upsert_symbol_day_index_rows(
    *,
    ...
    """
    Upsert per-(symbol, trading_day, ticks_lake, lake_version) coverage rows.
    """
    idx = str(index_table).strip() or INDEX_TABLE_V2
    rs = list(rows)
    if not rs:
    ...
    sql = (
        f"INSERT INTO {idx} "
        "(ts, symbol, trading_day, ticks_lake, lake_version, rows_total, first_datetime_ns, last_datetime_ns, l5_present, "
...
def upsert_no_data_days(
    *,
    ...
    params: list[tuple[Any, ...]] = []
    for d in days:
        params.append((_day_to_ts_utc(d), str(symbol), str(d), str(ticks_lake), str(lake_version), str(reason), now))
    sql = f"INSERT INTO {nd} (ts, symbol, trading_day, ticks_lake, lake_version, reason, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s)"
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
        with conn.cursor() as cur:
            cur.executemany(sql, params)
    return int(len(params))
...
def query_present_trading_days(
    *,
    ...
    where = [f"symbol IN ({placeholders})", "ticks_lake = %s", "lake_version = %s", "rows_total > 0"]
    params: list[Any] = list(syms) + [tl, lv]
...
def list_symbols_from_index(
    *,
    ...
    where = ["ticks_lake = %s", "lake_version = %s", "rows_total > 0"]
    params: list[Any] = [tl, lv]
    if prefix:
        p = str(prefix).strip()
        if p:
...
def query_index_coverage_rows(
    *,
    ...
    base_where = ["ticks_lake = %s", "lake_version = %s", "rows_total > 0"]
    base_params: list[Any] = [tl, lv]
...
def list_present_trading_days(
    *,
    ...
    sql = (
        ...
        "WHERE symbol = %s AND ticks_lake = %s AND lake_version = %s AND rows_total > 0 "
        "AND ts >= %s AND ts <= %s"
    )
    out: set[date] = set()
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
        with conn.cursor() as cur:
            cur.execute(sql, [sym, tl, lv, s_ts, e_ts])
            for row in cur.fetchall():
                try:
...
def fetch_present_days_by_symbol(
    *,
    ...
    sql = (
        ...
        f"FROM {idx} "
        f"WHERE symbol IN ({placeholders}) AND ticks_lake=%s AND lake_version=%s AND rows_total > 0 "
        "AND ts >= %s AND ts <= %s"
    )
    params: list[Any] = list(syms) + [tl, lv, s_ts, e_ts]
...
def list_no_data_trading_days(
    *,
    ...
    sql = (
        ...
        "WHERE symbol = %s AND ticks_lake = %s AND lake_version = %s "
        "AND ts >= %s AND ts <= %s"
    )
    out: set[date] = set()
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
...
def get_symbol_day_index_row(
    *,
    ...
    """
    Fetch the current index row for a (symbol, trading_day, ticks_lake, lake_version) key.
    """
    idx = str(index_table).strip() or INDEX_TABLE_V2
    sym = str(symbol).strip()
    ...
    sql = (
        ...
        f"FROM {idx} "
        "WHERE symbol=%s AND trading_day=%s AND ticks_lake=%s AND lake_version=%s "
        "LIMIT 1"
    )
    sql_legacy = (
        f"SELECT rows_total, first_datetime_ns, last_datetime_ns, l5_present "
    ...
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
        with conn.cursor() as cur:
            has_row_hash = True
            try:
                cur.execute(sql, [sym, td, tl, lv])
            except Exception:
                has_row_hash = False
                cur.execute(sql_legacy, [sym, td, tl, lv])
            row = cur.fetchone()
            if not row:
                return None
            out: dict[str, Any] = {
...
def _parallel_bootstrap_worker(args: tuple) -> dict[str, Any]:
    ...
    Args:
        args: Tuple of (cfg_dict, ticks_table, symbols, lake_version, ticks_lake, index_table, connect_timeout_s)

    Returns:
        dict with rows_upserted and any errors
    """
    from ghtrader.questdb_queries import QuestDBQueryConfig

    cfg_dict, ticks_table, symbols, lake_version, ticks_lake, index_table, connect_timeout_s = args
...
def bootstrap_symbol_day_index_parallel(
    *,
    ...
    Args:
        ...
        lake_version: Lake version (v2)
        ticks_lake: Ticks lake type (raw/main_l5)
    ...
    # Prepare worker args
    worker_args = [
        (cfg_dict, tbl, batch, lv, tl, idx, connect_timeout_s)
