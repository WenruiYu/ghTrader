...
from ghtrader.questdb_queries import QuestDBQueryConfig, _connect
...
def ensure_features_tables(
    *,
    ...
    CREATE TABLE IF NOT EXISTS {ft} (
      ...
      DEDUP UPSERT KEYS(ts, symbol, ticks_lake, lake_version, build_id, row_hash)
    """
    ...
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
        ...
        with conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    f"ALTER TABLE {ft} DEDUP ENABLE UPSERT KEYS(ts, symbol, ticks_lake, lake_version, build_id, row_hash)"
                )
            except Exception:
                pass
    ...
    *,
    ...
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
        ...
        with conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    f"ALTER TABLE {lt} DEDUP ENABLE UPSERT KEYS(ts, symbol, ticks_lake, lake_version, build_id, row_hash)"
                )
            except Exception:
                pass
...
def insert_feature_build(
    *,
    ...
    sql = (
        f"INSERT INTO {bt} "
        "(ts, symbol, ticks_lake, lake_version, build_id, factors_hash, factors, schema_hash, schedule_hash, rows_total, first_day, last_day) "
...
def insert_label_build(
    *,
    ...
    sql = (
        f"INSERT INTO {bt} "
        "(ts, symbol, ticks_lake, lake_version, build_id, horizons, threshold_k, price_tick, schedule_hash, rows_total, first_day, last_day) "
        "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    )
    params = (
        now,
...
def get_latest_feature_build(
    *,
    ...
    sql = (
        ...
        f"FROM {bt} WHERE symbol=%s AND ticks_lake=%s AND lake_version=%s "
        "ORDER BY ts DESC LIMIT 1"
    )
    with _connect(cfg, connect_timeout_s=connect_timeout_s) as conn:
        with conn.cursor() as cur:
            cur.execute(sql, [str(symbol), str(ticks_lake), str(lake_version)])
            row = cur.fetchone()
            if not row:
                return None
            return {
